...
<div class="workers">
            ...
            if ($stmt1->rowCount() > 0) {
                foreach ($result as $row1) {
                    if ($row1["status"] == 1) {
                        ...
                        echo '<form action="time_duration.php" method="post">
                            <input hidden value='.$id_user.' name="id_user" id="id_user">
                            <div class="card-body"><button type="submit" class="btn" id="buttonw" name="buttonw"" style="background-color: #9E8A78"><a style="text-decoration: none; color: #000000;">Időpont választás</a></button></form></div></div>';
                    }
                }
            }
        }
    ...
    if (!$hasStatusOne) {
        ...
        echo "<h1>Nincs megjeleníthető fodrász</h1>";
        echo "</div>";
    }
